class BulkQuery private constructor(private val graphName: String, private val begin: Boolean, private val typeNodesList: List<TypeNodes>, private val typeEdgesList: List<TypeEdges>) {
    ...
    data class Builder(
        ...
        fun addTypeNode(type: String, typeNode: TypeNodes) = apply {
            if (type !in typeNodes)
                this.typeNodes += type to typeNode
        }
        fun addNodeToTypeNode(type: String, node: Node) = apply {
            if (type in this.typeNodes)
                this.typeNodes[type]?.addNodes(node)
        }

        fun addTypeEdge(type: String, typeEdge: TypeEdges) = apply {
            if (type !in typeEdges)
                this.typeEdges += type to typeEdge
        }
        fun addEdgeToTypeEdge(type: String, edge: Edge) = apply {
            if (type in this.typeEdges)
                this.typeEdges[type]?.addEdges(edge)
        }
        ...
        fun build() = BulkQuery(graphName!!, begin, typeNodes.values.toList(), typeEdges.values.toList())
    }

    fun generate_query_args(): Array<ByteArray> {
        var args: Array<ByteArray> = arrayOf()

        args += graphName.toByteArray()
        if (begin)
            args += "BEGIN".toByteArray()
        ...
        this.typeNodesList.forEach {
            args += it.toBinary()
        }
        this.typeEdgesList.forEach {
            args += it.toBinary()
        }
    ...
    override fun toString(): String {
        return generate_query_args().joinToString(separator = " - ") { bytes -> bytes.toHex() }
    }
}