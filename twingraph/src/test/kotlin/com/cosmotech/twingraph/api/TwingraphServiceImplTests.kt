...
class TwingraphServiceImplTests {
  ...
  // @Test
  // Test not finished yet, but it is working, it shows requests generated by the processBulkQuery
  fun `test processBulkQuery - should return empty list when no data found`() {

    val fileName = this::class.java.getResource("/User.csv")?.file
    val file = File(fileName)
    val cypherQuery = "CREATE (:Person {id: toInteger(\$id), name: \$name, rank: toInteger(\$rank)})"

    var count = 0;
    val pagination = 15
    var stringBuilder = StringBuilder()
    twingraphServiceImpl.processBulkQuery(file.inputStream(), "node", cypherQuery) { query, predicate ->
      ...
      if (count == pagination) {
        ...
        println("$predicate $stringBuilder")
        stringBuilder = StringBuilder()
      }
    }
    if (count > 0) {
      stringBuilder.deleteCharAt(stringBuilder.length - 1)
      println("${cypherQuery.cypherAction()} $stringBuilder")
    }
  }

  //  @Test
  // File to generate bulk data for testing
  fun generateUserFile() {
    val file = File("/tmp/UserBulk.csv")
    val stringBuilder = StringBuilder("id,name,rank\n")
    for (i in 1..300000) {
      stringBuilder.append("$i,\"User$i\",${Random.nextInt(0, 50)}\n")
    }
