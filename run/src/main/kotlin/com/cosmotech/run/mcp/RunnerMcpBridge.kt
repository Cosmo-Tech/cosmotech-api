...
class RunnerMcpBridge(private val runnerApiService: RunnerApiService) {
  ...
  fun createRunner(
      ...
      @ToolParam(description = "The runner description", required = false)
      description: String? = null,
      @ToolParam(
          description = "The dataset list ids",
          required = false,
      )
      datasetList: MutableList<String>? = arrayListOf(),
      @ToolParam(
          description =
              "The parameters map: keys are parameterId and values are parameterValues id",
          required = false,
      )
      parametersMaps: Map<String, String>? = emptyMap(),
  ): String {
    val runnerCreateRequest =
        RunnerCreateRequest(
            ...
            parametersValues =
                parametersMaps
                    ?.map {
                      RunnerRunTemplateParameterValue(it.key, it.value)
                    }
                    ?.toMutableList(),
        )
