...
class RunnerServiceIntegrationTest : CsmTestBase() {
  ...
  // TODO: test create runner with inherited dataset parameter from parent and workspace (si
  // modification de la solution entre creation parent et child)
  @Test
  fun `test create runner with inherited DB dataset parameter from parent with solution modified in between`() {
    ...
    solutionApiService.createSolutionParameter(
        ...
        solutionSaved.id,
        RunTemplateParameterCreateRequest(
            id = "datasetPartParam2",
            defaultValue = "this_value_is_ignored",
            varType = DATASET_PART_VARTYPE_DB))
    solutionApiService.updateSolutionParameterGroup(
        organizationSaved.id,
        solutionSaved.id,
        "defaultDatasetPartParameterGroup",
        RunTemplateParameterGroupUpdateRequest(
            parameters = mutableListOf("datasetPartParam", "datasetPartParam2")))
                    ...
                    WorkspaceSolution(
                        ...
                        defaultParameterValues =
                            mutableMapOf(
                                ...
                                "datasetPartParam2" to newWorkspaceDatasetPart.id))))
    ...
    val datasetPartIdCustomers5FromChild =
        childRunnerDataset.parts.first { it.sourceName == CUSTOMERS_5_LINES_FILE_NAME }.id

    val datasetPartIdCustomersFromChild =
        childRunnerDataset.parts.first { it.sourceName == CUSTOMERS_FILE_NAME }.id

    val childRunnerDatasetPartCustomers5Content =
        datasetApiService.downloadDatasetPart(
            organizationSaved.id,
            workspaceSaved.id,
            childRunnerDataset.id,
            datasetPartIdCustomers5FromChild)

    val childRunnerDatasetPartCustomersContent =
        datasetApiService.downloadDatasetPart(
            organizationSaved.id,
            workspaceSaved.id,
            childRunnerDataset.id,
            datasetPartIdCustomersFromChild)

    var expectedText = FileInputStream(customers5File).bufferedReader().use { it.readText() }
    var retrievedText =
        InputStreamResource(childRunnerDatasetPartCustomers5Content)
            .inputStream
            .bufferedReader()
            .use { it.readText() }
    assertEquals(expectedText, retrievedText)

    expectedText = FileInputStream(customersFile).bufferedReader().use { it.readText() }
    retrievedText =
        InputStreamResource(childRunnerDatasetPartCustomersContent)
            .inputStream
            .bufferedReader()
            .use { it.readText() }
    assertEquals(expectedText, retrievedText)
