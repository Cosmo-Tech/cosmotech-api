...
class GenerateSwaggerTests {
    ...
    fun `generate openapi spec`() {
        ...
        val apiDocsResponseEntity = this.restTemplate.getForEntity("http://localhost:$port/v3/api-docs", String::class.java)
        ...
        val outputFile = File(System.getProperty("java.io.tmpdir", "/tmp"), "cosmotech-api.openapi.json")
        outputFile.writeText(apiDocs, StandardCharsets.UTF_8)
        logger.info("/!\\ Cosmo Tech resulting OpenAPI specification file generated at: ${outputFile.absolutePath}")
    }
}