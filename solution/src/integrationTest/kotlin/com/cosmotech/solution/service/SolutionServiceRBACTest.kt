...
class SolutionServiceRBACTest : CsmTestBase() {
  ...
  @TestFactory
  fun `test Organization RBAC getSolutionFile`() =
      mapOf(
              ROLE_VIEWER to false,
              ROLE_EDITOR to false,
              ROLE_USER to false,
          ...
          .map { (role, shouldThrow) ->
            DynamicTest.dynamicTest("Test Organization RBAC getSolutionFile : $role") {
              every { getCurrentAccountIdentifier(any()) } returns CONNECTED_ADMIN_USER
              ...
              if (shouldThrow) {
                val exception =
                    assertThrows<CsmAccessForbiddenException> {
                      solutionApiService.getSolutionFile(
                          organizationSaved.id, solutionSaved.id, fileName)
                    }
                assertEquals(
                    "RBAC ${organizationSaved.id} - User does not have permission $PERMISSION_READ",
                    exception.message)
  ...
  @TestFactory
  fun `test Solution RBAC getSolutionFile`() =
      mapOf(
              ROLE_VIEWER to false,
              ROLE_EDITOR to false,
              ROLE_USER to false,
          ...
          .map { (role, shouldThrow) ->
            DynamicTest.dynamicTest("Test Solution RBAC getSolutionFile : $role") {
              every { getCurrentAccountIdentifier(any()) } returns CONNECTED_ADMIN_USER
  ...
  @TestFactory
  fun `test Organization RBAC createSolutionFile`() =
      mapOf(
              ROLE_VIEWER to false,
              ROLE_EDITOR to false,
              ROLE_USER to false,
          ...
          .map { (role, shouldThrow) ->
            DynamicTest.dynamicTest("Test Organization RBAC createSolutionFile : $role") {
              every { getCurrentAccountIdentifier(any()) } returns CONNECTED_ADMIN_USER
  ...
  @TestFactory
  fun `test Solution RBAC createSolutionFile`() =
      mapOf(
              ROLE_VIEWER to true,
              ROLE_EDITOR to false,
              ROLE_USER to true,
          ...
          .map { (role, shouldThrow) ->
            DynamicTest.dynamicTest("Test Solution RBAC createSolutionFile : $role") {
              every { getCurrentAccountIdentifier(any()) } returns CONNECTED_ADMIN_USER
  ...
  @TestFactory
  fun `test Organization RBAC deleteSolutionFile`() =
      mapOf(
              ROLE_VIEWER to false,
              ROLE_EDITOR to false,
              ROLE_USER to false,
          ...
          .map { (role, shouldThrow) ->
            DynamicTest.dynamicTest("Test Organization RBAC deleteSolutionFile : $role") {
              every { getCurrentAccountIdentifier(any()) } returns CONNECTED_ADMIN_USER
              ...
              if (shouldThrow) {
                val exception =
                    assertThrows<CsmAccessForbiddenException> {
                      solutionApiService.deleteSolutionFile(
                          organizationSaved.id, solutionSaved.id, fileName)
                    }
                assertEquals(
                    "RBAC ${organizationSaved.id} - User does not have permission $PERMISSION_READ",
  ...
  @TestFactory
  fun `test Solution RBAC deleteSolutionFile`() =
      mapOf(
              ROLE_VIEWER to true,
              ROLE_EDITOR to false,
              ROLE_USER to true,
          ...
          .map { (role, shouldThrow) ->
            DynamicTest.dynamicTest("Test Solution RBAC deleteSolutionFile : $role") {
              every { getCurrentAccountIdentifier(any()) } returns CONNECTED_ADMIN_USER
  ...
  @TestFactory
  fun `test Organization RBAC deleteSolutionFiles`() =
      mapOf(
              ROLE_VIEWER to false,
              ROLE_EDITOR to false,
              ROLE_USER to false,
          ...
          .map { (role, shouldThrow) ->
            DynamicTest.dynamicTest("Test Organization RBAC deleteSolutionFiles : $role") {
              every { getCurrentAccountIdentifier(any()) } returns CONNECTED_ADMIN_USER
  ...
  @TestFactory
  fun `test Solution RBAC deleteSolutionFiles`() =
      mapOf(
              ROLE_VIEWER to true,
              ROLE_EDITOR to false,
              ROLE_USER to true,
          ...
          .map { (role, shouldThrow) ->
            DynamicTest.dynamicTest("Test Solution RBAC deleteSolutionFiles : $role") {
              every { getCurrentAccountIdentifier(any()) } returns CONNECTED_ADMIN_USER
