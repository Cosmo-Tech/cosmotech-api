...
class DatasetServiceIntegrationTest() : CsmTestBase() {
  ...
  val DATASET_ID = "datasetId"
  val DATASET_PART_ID = "datasetPartId"
  ...
  val DATASET_ID_MUST_NOT_BE_BLANK = "Dataset Id must not be blank"
  val DATASET_PART_ID_MUST_NOT_BE_BLANK = "Dataset Part Id must not be blank"
  ...
  @Test
  fun `getDataset with empty ids`() {
    var exception =
        assertThrows<IllegalStateException> {
          datasetApiService.getDataset("", WORKSPACE_ID, DATASET_ID)
        }
    assertEquals(ORGANIZATION_ID_MUST_NOT_BE_BLANK, exception.message)

    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.getDataset(ORGANIZATION_ID, "", DATASET_ID)
        }
    assertEquals(WORKSPACE_ID_MUST_NOT_BE_BLANK, exception.message)

    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.getDataset(ORGANIZATION_ID, WORKSPACE_ID, "")
        }
    assertEquals(DATASET_ID_MUST_NOT_BE_BLANK, exception.message)
  }

  @Test
  fun `createDataset with empty ids`() {
    var exception =
        assertThrows<IllegalStateException> {
          datasetApiService.createDataset(
              "", WORKSPACE_ID, DatasetCreateRequest(name = "test"), files = emptyArray())
        }
    assertEquals(ORGANIZATION_ID_MUST_NOT_BE_BLANK, exception.message)
    ...
    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.createDataset(
              ORGANIZATION_ID, "", DatasetCreateRequest(name = "test"), files = emptyArray())
        }
    assertEquals(WORKSPACE_ID_MUST_NOT_BE_BLANK, exception.message)
  }

  @Test
  fun `createDatasetAccessControl with empty ids`() {
    var exception =
        assertThrows<IllegalStateException> {
          datasetApiService.createDatasetAccessControl(
              "", WORKSPACE_ID, DATASET_ID, DatasetAccessControl(id = "test", role = ROLE))
        }
    assertEquals(ORGANIZATION_ID_MUST_NOT_BE_BLANK, exception.message)
    ...
    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.createDatasetAccessControl(
              ORGANIZATION_ID, "", DATASET_ID, DatasetAccessControl(id = "test", role = ROLE))
        }
    assertEquals(WORKSPACE_ID_MUST_NOT_BE_BLANK, exception.message)
    ...
    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.createDatasetAccessControl(
              ORGANIZATION_ID, WORKSPACE_ID, "", DatasetAccessControl(id = "test", role = ROLE))
        }
    assertEquals(DATASET_ID_MUST_NOT_BE_BLANK, exception.message)
  }

  @Test
  fun `createDatasetPart with empty ids`() {
    var exception =
        assertThrows<IllegalStateException> {
          datasetApiService.createDatasetPart(
              "",
              ...
              DatasetPartCreateRequest(name = "test", sourceName = "sourceName"))
        }
    assertEquals(ORGANIZATION_ID_MUST_NOT_BE_BLANK, exception.message)
  ...
  @Test
  fun `deleteDataset with empty ids`() {
    var exception =
        assertThrows<IllegalStateException> {
          datasetApiService.deleteDataset("", WORKSPACE_ID, DATASET_ID)
        }
    assertEquals(ORGANIZATION_ID_MUST_NOT_BE_BLANK, exception.message)

    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.deleteDataset(ORGANIZATION_ID, "", DATASET_ID)
        }
    assertEquals(WORKSPACE_ID_MUST_NOT_BE_BLANK, exception.message)

    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.deleteDataset(ORGANIZATION_ID, WORKSPACE_ID, "")
        }
    assertEquals(DATASET_ID_MUST_NOT_BE_BLANK, exception.message)
  }

  @Test
  fun `deleteDatasetAccessControl with empty ids`() {
    var exception =
        assertThrows<IllegalStateException> {
          datasetApiService.deleteDatasetAccessControl("", WORKSPACE_ID, DATASET_ID, IDENTITY_ID)
        }
    assertEquals(ORGANIZATION_ID_MUST_NOT_BE_BLANK, exception.message)

    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.deleteDatasetAccessControl(ORGANIZATION_ID, "", DATASET_ID, IDENTITY_ID)
        }
    assertEquals(WORKSPACE_ID_MUST_NOT_BE_BLANK, exception.message)
    ...
    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.deleteDatasetAccessControl(
              ORGANIZATION_ID, WORKSPACE_ID, "", IDENTITY_ID)
        }
    assertEquals(DATASET_ID_MUST_NOT_BE_BLANK, exception.message)
  }

  @Test
  fun `deleteDatasetPart with empty ids`() {
    var exception =
        assertThrows<IllegalStateException> {
          datasetApiService.deleteDatasetPart("", WORKSPACE_ID, DATASET_ID, DATASET_PART_ID)
        }
    assertEquals(ORGANIZATION_ID_MUST_NOT_BE_BLANK, exception.message)

    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.deleteDatasetPart(ORGANIZATION_ID, "", DATASET_ID, DATASET_PART_ID)
        }
    assertEquals(WORKSPACE_ID_MUST_NOT_BE_BLANK, exception.message)

    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.deleteDatasetPart(ORGANIZATION_ID, WORKSPACE_ID, "", DATASET_PART_ID)
        }
    assertEquals(DATASET_ID_MUST_NOT_BE_BLANK, exception.message)

    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.deleteDatasetPart(ORGANIZATION_ID, WORKSPACE_ID, DATASET_ID, "")
        }
    assertEquals(DATASET_PART_ID_MUST_NOT_BE_BLANK, exception.message)
  }

  @Test
  fun `downloadDatasetPart with empty ids`() {
    var exception =
        assertThrows<IllegalStateException> {
          datasetApiService.downloadDatasetPart("", WORKSPACE_ID, DATASET_ID, DATASET_PART_ID)
        }
    assertEquals(ORGANIZATION_ID_MUST_NOT_BE_BLANK, exception.message)

    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.downloadDatasetPart(ORGANIZATION_ID, "", DATASET_ID, DATASET_PART_ID)
        }
    assertEquals(WORKSPACE_ID_MUST_NOT_BE_BLANK, exception.message)

    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.downloadDatasetPart(ORGANIZATION_ID, WORKSPACE_ID, "", DATASET_PART_ID)
        }
    assertEquals(DATASET_ID_MUST_NOT_BE_BLANK, exception.message)

    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.downloadDatasetPart(ORGANIZATION_ID, WORKSPACE_ID, DATASET_ID, "")
        }
    assertEquals(DATASET_PART_ID_MUST_NOT_BE_BLANK, exception.message)
  }

  @Test
  fun `getDatasetAccessControl with empty ids`() {
    var exception =
        assertThrows<IllegalStateException> {
          datasetApiService.getDatasetAccessControl("", WORKSPACE_ID, DATASET_ID, IDENTITY_ID)
        }
    assertEquals(ORGANIZATION_ID_MUST_NOT_BE_BLANK, exception.message)

    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.getDatasetAccessControl(ORGANIZATION_ID, "", DATASET_ID, IDENTITY_ID)
        }
    assertEquals(WORKSPACE_ID_MUST_NOT_BE_BLANK, exception.message)

    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.getDatasetAccessControl(ORGANIZATION_ID, WORKSPACE_ID, "", IDENTITY_ID)
        }
    assertEquals(DATASET_ID_MUST_NOT_BE_BLANK, exception.message)
  }

  @Test
  fun `getDatasetPart with empty ids`() {
    var exception =
        assertThrows<IllegalStateException> {
          datasetApiService.getDatasetPart("", WORKSPACE_ID, DATASET_ID, DATASET_PART_ID)
        }
    assertEquals(ORGANIZATION_ID_MUST_NOT_BE_BLANK, exception.message)

    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.getDatasetPart(ORGANIZATION_ID, "", DATASET_ID, DATASET_PART_ID)
        }
    assertEquals(WORKSPACE_ID_MUST_NOT_BE_BLANK, exception.message)

    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.getDatasetPart(ORGANIZATION_ID, WORKSPACE_ID, "", DATASET_PART_ID)
        }
    assertEquals(DATASET_ID_MUST_NOT_BE_BLANK, exception.message)

    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.getDatasetPart(ORGANIZATION_ID, WORKSPACE_ID, DATASET_ID, "")
        }
    assertEquals(DATASET_PART_ID_MUST_NOT_BE_BLANK, exception.message)
  }

  @Test
  fun `listDatasetParts with empty ids`() {
    var exception =
        assertThrows<IllegalStateException> {
          datasetApiService.listDatasetParts("", WORKSPACE_ID, DATASET_ID, null, null)
        }
    assertEquals(ORGANIZATION_ID_MUST_NOT_BE_BLANK, exception.message)

    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.listDatasetParts(ORGANIZATION_ID, "", DATASET_ID, null, null)
        }
    assertEquals(WORKSPACE_ID_MUST_NOT_BE_BLANK, exception.message)

    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.listDatasetParts(ORGANIZATION_ID, WORKSPACE_ID, "", null, null)
        }
    assertEquals(DATASET_ID_MUST_NOT_BE_BLANK, exception.message)
  }

  @Test
  fun `listDatasetSecurityUsers with empty ids`() {
    var exception =
        assertThrows<IllegalStateException> {
          datasetApiService.listDatasetSecurityUsers("", WORKSPACE_ID, DATASET_ID)
        }
    assertEquals(ORGANIZATION_ID_MUST_NOT_BE_BLANK, exception.message)

    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.listDatasetSecurityUsers(ORGANIZATION_ID, "", DATASET_ID)
        }
    assertEquals(WORKSPACE_ID_MUST_NOT_BE_BLANK, exception.message)

    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.listDatasetSecurityUsers(ORGANIZATION_ID, WORKSPACE_ID, "")
        }
    assertEquals(DATASET_ID_MUST_NOT_BE_BLANK, exception.message)
  }

  @Test
  fun `listDatasets with empty ids`() {
    var exception =
        assertThrows<IllegalStateException> {
          datasetApiService.listDatasets("", WORKSPACE_ID, null, null)
        }
    assertEquals(ORGANIZATION_ID_MUST_NOT_BE_BLANK, exception.message)

    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.listDatasets(ORGANIZATION_ID, "", null, null)
        }
    assertEquals(WORKSPACE_ID_MUST_NOT_BE_BLANK, exception.message)
  }

  @Test
  fun `queryData with empty ids`() {
    var exception =
        assertThrows<IllegalStateException> {
          datasetApiService.queryData(
              "",
  ...
  @Test
  fun `replaceDatasetPart with empty ids`() {
    var exception =
        assertThrows<IllegalStateException> {
          datasetApiService.replaceDatasetPart(
              "",
  ...
  @Test
  fun `searchDatasetParts with empty ids`() {
    var exception =
        assertThrows<IllegalStateException> {
          datasetApiService.searchDatasetParts(
              "", WORKSPACE_ID, DATASET_ID, emptyList(), null, null)
        }
    assertEquals(ORGANIZATION_ID_MUST_NOT_BE_BLANK, exception.message)
    ...
    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.searchDatasetParts(
              ORGANIZATION_ID, "", DATASET_ID, emptyList(), null, null)
        }
    assertEquals(WORKSPACE_ID_MUST_NOT_BE_BLANK, exception.message)
    ...
    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.searchDatasetParts(
              ORGANIZATION_ID, WORKSPACE_ID, "", emptyList(), null, null)
        }
    assertEquals(DATASET_ID_MUST_NOT_BE_BLANK, exception.message)
  }

  @Test
  fun `searchDatasets with empty ids`() {
    var exception =
        assertThrows<IllegalStateException> {
          datasetApiService.searchDatasets("", WORKSPACE_ID, emptyList(), null, null)
        }
    assertEquals(ORGANIZATION_ID_MUST_NOT_BE_BLANK, exception.message)

    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.searchDatasets(ORGANIZATION_ID, "", emptyList(), null, null)
        }
    assertEquals(WORKSPACE_ID_MUST_NOT_BE_BLANK, exception.message)
  }

  @Test
  fun `updateDataset with empty ids`() {
    var exception =
        assertThrows<IllegalStateException> {
          datasetApiService.updateDataset(
              "", WORKSPACE_ID, DATASET_ID, DatasetUpdateRequest(), files = emptyArray())
        }
    assertEquals(ORGANIZATION_ID_MUST_NOT_BE_BLANK, exception.message)
    ...
    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.updateDataset(
              ORGANIZATION_ID, "", DATASET_ID, DatasetUpdateRequest(), files = emptyArray())
        }
    assertEquals(WORKSPACE_ID_MUST_NOT_BE_BLANK, exception.message)
  }

  @Test
  fun `updateDatasetAccessControl with empty ids`() {
    var exception =
        assertThrows<IllegalStateException> {
          datasetApiService.updateDatasetAccessControl(
              "", WORKSPACE_ID, DATASET_ID, "test", DatasetRole(ROLE))
        }
    assertEquals(ORGANIZATION_ID_MUST_NOT_BE_BLANK, exception.message)
    ...
    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.updateDatasetAccessControl(
              ORGANIZATION_ID, "", DATASET_ID, "test", DatasetRole(ROLE))
        }
    assertEquals(WORKSPACE_ID_MUST_NOT_BE_BLANK, exception.message)
    ...
    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.updateDatasetAccessControl(
              ORGANIZATION_ID, WORKSPACE_ID, "", "test", DatasetRole(ROLE))
        }
    assertEquals(DATASET_ID_MUST_NOT_BE_BLANK, exception.message)
  }

  @Test
  fun `updateDatasetDefaultSecurity with empty ids`() {
    var exception =
        assertThrows<IllegalStateException> {
          datasetApiService.updateDatasetDefaultSecurity(
              "", WORKSPACE_ID, DATASET_ID, DatasetRole(ROLE))
        }
    assertEquals(ORGANIZATION_ID_MUST_NOT_BE_BLANK, exception.message)
    ...
    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.updateDatasetDefaultSecurity(
              ORGANIZATION_ID, "", DATASET_ID, DatasetRole(ROLE))
        }
    assertEquals(WORKSPACE_ID_MUST_NOT_BE_BLANK, exception.message)
    ...
    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.updateDatasetDefaultSecurity(
              ORGANIZATION_ID, WORKSPACE_ID, "", DatasetRole(ROLE))
        }
    assertEquals(DATASET_ID_MUST_NOT_BE_BLANK, exception.message)
  }

  @Test
  fun `updateDatasetPart with empty ids`() {
    var exception =
        assertThrows<IllegalStateException> {
          datasetApiService.updateDatasetPart(
              "", WORKSPACE_ID, DATASET_ID, DATASET_PART_ID, DatasetPartUpdateRequest())
        }
    assertEquals(ORGANIZATION_ID_MUST_NOT_BE_BLANK, exception.message)
    ...
    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.updateDatasetPart(
              ORGANIZATION_ID, "", DATASET_ID, DATASET_PART_ID, DatasetPartUpdateRequest())
        }
    assertEquals(WORKSPACE_ID_MUST_NOT_BE_BLANK, exception.message)
    ...
    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.updateDatasetPart(
              ORGANIZATION_ID, WORKSPACE_ID, "", DATASET_PART_ID, DatasetPartUpdateRequest())
        }
    assertEquals(DATASET_ID_MUST_NOT_BE_BLANK, exception.message)
    ...
    exception =
        assertThrows<IllegalStateException> {
          datasetApiService.updateDatasetPart(
              ORGANIZATION_ID, WORKSPACE_ID, DATASET_ID, "", DatasetPartUpdateRequest())
        }
    assertEquals(DATASET_PART_ID_MUST_NOT_BE_BLANK, exception.message)
  }
