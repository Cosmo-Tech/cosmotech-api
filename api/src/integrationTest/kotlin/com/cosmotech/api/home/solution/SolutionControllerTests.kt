...
class SolutionControllerTests : ControllerTestBase() {
  ...
  fun get_solution_members() {
    val solutionSecurity =
        SolutionSecurity(
            ...
            accessControlList =
                mutableListOf(
                    SolutionAccessControl(id = PLATFORM_ADMIN_EMAIL, role = ROLE_ADMIN),
                    SolutionAccessControl(id = ORGANIZATION_USER_EMAIL, role = ROLE_EDITOR),
                    SolutionAccessControl(id = PUBLIC_GROUP_NAME, role = ROLE_VIEWER),
                    SolutionAccessControl(id = UNKNOWN_IDENTITY, role = ROLE_EDITOR),
                    SolutionAccessControl(id = PRIVATE_GROUP_NAME, role = ROLE_VIEWER),
                ),
        )
    val solutionId =
        createSolutionAndReturnId(
    ...
    mvc.perform(
            get("/organizations/$organizationId/solutions/$solutionId/members")
                .withPlatformAdminHeader()
                .accept(MediaType.APPLICATION_JSON)
        )
        .andExpect(status().is2xxSuccessful)
        ...
        .andDo(MockMvcResultHandlers.print())
        .andDo(document("organizations/{organization_id}/solutions/{solution_id}/members/GET"))
  }
}
