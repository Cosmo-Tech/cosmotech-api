...
class MetaControllerTests : ControllerTestBase() {
  ...
  fun about() {
    mvc.perform(get("/about")
      ...
      .andDo {
        val aboutVersion = JSONObject(it.getResponse().getContentAsString()).getJSONObject("version")

        var expectedRelease = "${aboutVersion.getInt("major")}.${aboutVersion.getInt("minor")}.${aboutVersion.getInt("patch")}"
