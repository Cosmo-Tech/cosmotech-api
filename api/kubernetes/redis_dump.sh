...
usage() {
  echo "usage: $0 [-s] [-r] [-x] [-n namespace] [-d redis_pod] [-h redis_host] [-a redis_password] [-p redis_port]"
...
check_prerequisites() {
  DUMP_USER=$(stat -c '%U' ${REDIS_MOUNT_DUMP})
  if [[ $DUMP_USER == "root" ]]; then
    echo "ERROR: Redis dump file ${REDIS_MOUNT_DUMP} is owned by root user, please change the owner to the current user"
    exit 1
  fi
  ...
  FOUND_POD=$(kubectl get pod --no-headers -n ${NAMESPACE} ${REDIS_POD} | awk '{print $1}')
  check_not_empty "$FOUND_POD" "Redis pod ${REDIS_POD} not found on namespace ${NAMESPACE}"
  echo "Redis pod found: ${FOUND_POD}"
...
save_database() {
  ...
  echo "Connecting to Redis: ${redis_connect} from context $CURRENT_CONTEXT - ${NAMESPACE}/${FOUND_POD}"
  RESULT=$(kubectl exec -it ${FOUND_POD} -n ${NAMESPACE} -- ${redis_connect} bgsave)
...
REDIS_MOUNT_DUMP="$HOME/data/dump.rdb"
NAMESPACE="phoenix"
REDIS_HOST="localhost"
REDIS_POD="cosmotechredis-master-0"
REDIS_PORT="6379"
REDIS_PASSWORD=""
COMPLETION=false


while getopts 'srxyhn:d:h:a:p:f:' args
do
  case $args in
    s) ACTION=SAVE ;;
    r) ACTION=RESTORE ;;
